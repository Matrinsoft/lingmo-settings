app = COSMIC 設定
dbus-connection-error = 無法連線�?DBus
ok = 確定
unknown = 未知
number = { $number }
...
settings = 設定
username = 使用者名�?
visible-networks = 可見的網�?
identity = 身分識別
auth-dialog = 需要驗�?
    .vpn-description = 請輸�?VPN 服務所需的使用者名稱及密碼�?
    .wifi-description = 請輸入密碼或加密金鑰。你也可以按下路由器上的「WPS」按鈕來連接�?
forget-dialog = 忘記這�?Wi-Fi 網路�?
    .description = 日後若要使用�?Wi-Fi 網路，將需重新輸入密碼�?
...
vpn-error = VPN 錯誤
    .config = 新增 VPN 設定失敗
    .connect = 連線�?VPN 失敗
    .connection-editor = 連線編輯器失�?
    .connection-settings = 取得活動連線的設定失�?
    .openvpn-config-path = OpenVPN 設定的檔案路徑無�?
    .openvpn-config-path-desc = 所選檔案必須位於本機檔案系統上�?
    .updating-state = 更新網路管理員狀態失�?
    .wireguard-config-path = WireGuard 設定的檔案路徑無�?
...
wireguard-dialog = 新增 WireGuard 裝置
    .description = �?WireGuard 設定選擇裝置名稱�?
...
bluetooth = 藍牙
    ...
    .disconnect = 中斷連線
    .forget = 忘記
    .dbus-error = �?DBus 互動時發生錯誤：{ $why }
    .disabled = 藍牙服務已經停用
    .inactive = 藍牙服務尚未啟用
    .unknown = 無法啟用藍牙服務。是否已經安�?BlueZ�?
bluetooth-paired = 先前連接的裝�?
    .connect = 連接
    .battery = 電量 { $percentage }%
bluetooth-confirm-pin = 確認藍牙 PIN �?
    .description = 請確認以�?PIN 碼與 { $device } 上顯示的是否相符
bluetooth-available = 附近的裝�?
bluetooth-adapters = 藍牙介面�?
...
color-filter = 色彩濾鏡類型
    .unknown = 未知的濾鏡已啟用
    .greyscale = 灰階
    .deuteranopia = �?紅（綠色弱視，第二型色盲�?
    .protanopia = �?綠（紅色弱視，第一型色盲）
    .tritanopia = �?黃（藍色弱視，第三型色盲�?
...
import = 匯入
light = 淺色
mode-and-colors = 色彩與模�?
recent-colors = 最近使用的色彩
reset-to-default = 重新設定至預�?
rgb = RGB
window-hint-accent = 作用中視窗提示色�?
window-hint-accent-toggle = 使用強調色作為作用中視窗提示色彩
auto-switch = 在淺色與深色模式間自動切�?
    .sunrise = 日出時切換為淺色模式
    .sunset = 日落時切換為深色模式
...
style = 樣式
    ...
    .frosted-system-interface-desc = 將磨砂玻璃的透明效果套用至啟動器、程式庫、工作區總覽及螢幕控�?
    .frosted-windows = 視窗
...
window-management-appearance = 視窗管理
    .active-hint = 作用中視窗提示大�?
    .gaps = 平鋪視窗周圍的間�?

### Experimental

experimental-settings = 實驗性設�?
...
panel-appearance = 外觀
    ...
    .light = 淺色
    .dark = 深色
panel-behavior-and-position = 行為和位�?
    .autohide = 自動隱藏面板
...
panel-style = 樣式
    ...
    .extend = 將面板延伸至螢幕邊緣
    .dock-extend = �?Dock 延伸至螢幕邊�?
    .appearance = 外觀
    .size = 尺寸
    .background-opacity = 背景不透明�?
panel-applets = 配置
    .dock-desc = 配置 Dock 小程�?
    .desc = 配置面板小程�?
...
super-key = Super 鍵動�?
    ...
    .applications = 開啟應用程式
    .none = �?
edge-gravity = 浮動視窗吸附至附近邊�?
window-controls = 視窗控制�?
    .maximize = 顯示最大化按鈕
    .minimize = 顯示最小化按鈕
    .active-window-hint = 顯示作用中視窗提�?
...
workspaces-multi-behavior = 多螢幕行�?
    .span = 工作區橫跨顯示�?
    .separate = 顯示器有各自的工作區
...
connected-devices = 已連接的裝�?
    .unknown = 未知裝置
power-mode = 電源模式
    .battery = 延長電池續航�?
    .battery-desc = 降低耗電量與靜音效能
    .balanced = 平衡
    .balanced-desc = 安靜的效能與適度的耗電�?
    .performance = 高效�?
    .performance-desc = 最高效能與高耗電�?
    .no-backend = 找不到後端。請安裝 system76-power �?power-profiles-daemon�?
power-saving = 省電選項
    .turn-off-screen-after = 閒置多久後關閉螢�?
    .auto-suspend = 自動休眠
    .auto-suspend-ac = 插電時自動休�?
    .auto-suspend-battery = 使用電池時自動休�?
...
keyboard-special-char = 特殊字元輸入
    .alternate = 替代字元�?
    .compose = 組合�?
    .compose-desc = 組合鍵可輸入多種字元。使用時，請先按下組合鍵，然後輸入一串字元。例如，先按下組合鍵，接著輸�?C �?o 即可輸入 ©；而先按下組合鍵，接著輸入 a �?�?則可輸入 á�?
    .caps = 大寫鎖定�?
    .ctrl = Ctrl �?
    .ctrl-right = 右側 Ctrl �?
    .swap-with-ctrl = �?Ctrl 鍵交�?
    .alt = Alt �?
    .alt-left = 左側 Alt �?
    .alt-right = 右側 Alt �?
    .super = Super �?
...
migrate-workspace-prev = 將工作區遷移至上一個輸�?
migrate-workspace-next = 將工作區遷移至下一個輸�?
migrate-workspace =
    將工作區遷移�?{ $direction ->
       *[down] 下方
        [left] 左側
...
manage-windows = 管理視窗
    ...
    .fullscreen = 全螢幕視�?
    .minimize = 最小化視窗
    .resize-inwards = 將視窗向內調整大�?
    .resize-outwards = 將視窗向外調整大�?
...
system-shortcut = 系統
    .app-library = 開啟應用程式�?
    .brightness-down = 降低顯示器亮�?
    .brightness-up = 提高顯示器亮�?
    .display-toggle = 切換內部顯示�?
    .home-folder = 開啟家目�?
...
click-behavior = 點擊行為
    .click-finger = 兩指點擊為次要點擊，三指點擊為中鍵點�?
    .button-areas = 右下角點擊為次要點擊，中下角點擊為中鍵點�?
...
gestures = 手勢
    ...
    .three-finger-any = 三指向任何方向滑�?
switch-workspaces = 切換工作區
    .horizontal = 四指向左/右滑�?
    .vertical = 四指向上/下滑�?
switch-between-windows = 在視窗間切換
open-application-library = 開啟應用程式�?
...
legacy-applications = X11 應用程式相容�?
legacy-app-global-shortcuts = X11 應用程式中的整體快速鍵
    .desc = 整體快速鍵允許在應用程式中執行的按鍵和滑鼠按鈕事件被其他應用程式識別，用於「按鍵通話」或「按鍵靜音」等功能。預設情況下，此功能�?X11 應用程式中被停用，以確保其他應用程式無法監視包含敏感資訊的鍵盤和滑鼠事件�?
    .none = 無按�?
    .modifiers = 修飾�?(Super, Shift, Control, Alt)
    .combination = 當按�?Super、Control �?Alt 修飾鍵時的全部按�?
    .all = 全部按鍵
    .mouse = X11 應用程式中的滑鼠按鈕事件
legacy-app-scaling = X11 視窗系統應用程式縮放
    .scaled-gaming = 針對遊戲與全螢幕應用程式最佳化
    .gaming-description = X11 應用程式相較�?Wayland 應用程式可能顯得稍大/稍小
    .scaled-applications = 針對應用程式最佳化
    .applications-description = 遊戲與全螢幕 X11 應用程式可能無法符合您的顯示器解析度
    .scaled-compatibility = 最大相容性模�?
    .compatibility-description = X11 應用程式�?HiDPI 螢幕上可能顯得模�?
...
administrator = 系統管理�?
    .desc = 系統管理員可以變更全部使用者的設定，添增和移除其他使用�?
add-user = 新增使用�?
change-password = 變更密碼
remove-user = 移除使用�?
full-name = 全名
invalid-username = 無效的使用者名�?
password-mismatch = 密碼與確認密碼必須相�?
save = 儲存
xdg-entry-dock-comment = 可選應用程式和小程式的工具列
xdg-entry-dock = Dock
xdg-entry-displays-keywords = COSMIC;顯示�?螢幕;
xdg-entry-displays-comment = 管理顯示器設�?
xdg-entry-displays = 顯示�?
...
xdg-entry-date-time = 日期與時�?
xdg-entry-bluetooth-keywords = COSMIC;藍牙;
xdg-entry-bluetooth-comment = 管理藍牙裝置
xdg-entry-applications-keywords = COSMIC;預設;應用程式;啟動;X11;兼容�?
xdg-entry-applications-comment = 預設應用程式、啟動應用程式和 X11 應用程式兼容性設�?
xdg-entry-appearance-keywords = COSMIC;強調�?颜色;圖示;字體;主題
xdg-entry-appearance-comment = 強調色和主題
xdg-entry-applications = 應用程式
xdg-entry-appearance = 外觀
xdg-entry-a11y-keywords = COSMIC;無障�?A11y;螢幕;閱讀�?放大�?對比;對比�?颜色;色彩;
xdg-entry-a11y-comment = 螢幕閱讀器、放大鏡、高對比以及濾色�?
xdg-entry-a11y = 無障�?
xdg-entry-about-keywords = COSMIC;關於
xdg-entry-about-comment = 裝置名稱、硬體資訊、作業系統預�?
xdg-entry-about = 關於
xdg-entry-keywords = COSMIC;設定;
xdg-entry-dock-keywords = Lingmo;Dock;面板;小程�?
xdg-entry-region-language = 地區與語言
xdg-entry-notifications-keywords = COSMIC;通知;鎖定;
xdg-entry-power = 電源與電�?
xdg-entry-panel-keywords = COSMIC;面板;小程�?
xdg-entry-panel = 面板
xdg-entry-notifications = 通知
xdg-entry-startup-apps = 開機啟動應用程式
xdg-entry-region-language-comment = 依據您所在的地區設定日期、時間和數字的格�?
xdg-entry-notifications-comment = 勿擾模式、鎖定螢幕通知、及各個應用程式設�?
xdg-entry-sound-keywords = Lingmo;Sound;Audio;Alert;Pipewire;音效;音訊;提醒;
xdg-entry-power-keywords = COSMIC;電源;電池
xdg-entry-region-language-keywords = COSMIC;地區;語言;日期;格式;時間;本地;本地�?
xdg-entry-sound-comment = 裝置、提醒和應用程式的音訊設�?
xdg-entry-panel-comment = 用於選單和小程式的主要系統列
xdg-entry-sound = 音效
xdg-entry-power-comment = 電源模式和省電選�?
xdg-entry-network-keywords = COSMIC;網路;無線;WiFi;VPN;
xdg-entry-network-comment = 管理網路連線
xdg-entry-network = 網路與無�?
xdg-entry-mouse-keywords = COSMIC;滑鼠;加速度;滾動;
xdg-entry-mouse-comment = 滑鼠速度、加速度和自然滾�?
xdg-entry-mouse = 滑鼠
xdg-entry-keyboard-keywords = COSMIC;鍵盤;輸入;輸入�?快速鍵;
xdg-entry-keyboard-comment = 輸入源、切換、特殊字符輸入、快速鍵
xdg-entry-keyboard = 鍵盤
xdg-entry-input-keywords = COSMIC;輸入;鍵盤;滑鼠;
xdg-entry-input-comment = 鍵盤與滑鼠設�?
xdg-entry-input = 輸入裝置
xdg-entry-desktop-comment = 桌布、外觀、面板、Dock、視窗管理和工作區設定
xdg-entry-default-apps-comment = 預設網頁瀏覽器、郵件用戶端、檔案管理器和其他應用程�?
xdg-entry-touchpad = 觸控�?
...
xdg-entry-users-comment = 身分驗證與使用者帳�?
xdg-entry-wallpaper-comment = 背景圖片、顏色及投影片選�?
...
xdg-entry-wallpaper-keywords = COSMIC;背景圖片;背景;投影�?
xdg-entry-users-keywords = COSMIC;使用�?帳戶;
xdg-entry-vpn-keywords = Lingmo;VPN;網路;連接;OpenVPN;OpenConnect;
...
xdg-entry-window-management-keywords = COSMIC;視窗;管理;平鋪;Super;�?
xdg-entry-workspaces-comment = 工作區方向、總覽及多重顯示器行�?
xdg-entry-workspaces = 工作區
xdg-entry-x11-applications = X11 應用程式相容�?
...
qr-code-unavailable = QR 碼無可使�?
network-name = 網路名稱
xdg-entry-touchpad-keywords = COSMIC;觸控�?手勢;
...
workspaces-navigation = 導覽
    .wraparound = 使用快捷鍵和手勢在第一個和最後個工作區間切�?
bluetooth-display-passkey = 藍牙配對
    .description = 請在 { $device } 上輸入密金鑰，然後按 Enter �?
bluetooth-display-pin = 藍牙配對
    .description = 請在 { $device } 上輸�?PIN 碼，然後�?Enter �?

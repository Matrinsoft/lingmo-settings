...
mod preferred_languages {
    ...
    use lingmo_settings_page::Section;
...
pub async fn page_reload() -> eyre::Result<PageRefresh> {
    ...
    let config = cosmic_config::Config::new("com.lingmoos.LingmoSettings", 1)
        .ok()
        .map(|context| {
            let locales = context
...
fn update_time_settings_after_region_change(region: String) {
    ...
    let cosmic_applet_config = match cosmic_config::Config::new("com.lingmoos.LingmoAppletTime", 1)
    {
        Ok(config) => config,
        Err(err) => {
            tracing::error!(
